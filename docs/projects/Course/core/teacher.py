...
def teacher_login():
    ...
    print('老师登录')
    while True:
        name = input('pelase input your name:').strip()
        if name == 'q':
            break
    ...
    while True:
        ...
        if choice.isdigit():
            ...
            if choice >= 0 and choice < len(course_list):
                teacher_interface.choose_course(
                    teacher_info['name'], course_list[choice]
                )
                break
            else:
                print('请选择已经存在的课程')
