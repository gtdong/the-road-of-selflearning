...
## 一. 数据库的介绍？
```
之前， 我们存取数据的时候， 使用的是Excel表格来进行管理， 但是， 随着数据量的增多，使用Excel表格就不行了， 而且我么还有
一些高级的查询，需要工具软件来帮助我们取完成,这个软件就是数据库软件
	
----->>: 数据库就是一个帮助我们进行数据管理的软件（工具）
```
[维基百科](https://zh.wikipedia.org/wiki/%E6%95%B0%E6%8D%AE%E5%BA%93)
## 二. 数据库的分类
```
a. 关系型数据库
		1. 有约束
